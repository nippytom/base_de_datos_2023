...
SELECT nombre FROM usuarios(SELECT usuario_id, SUM(dinero) FROM usuariosmisiones 
JOIN misiones ON misiones.id = usuariosmisiones.mision_id WHERE terminado = TRUE GROUP BY usuario_id) AS t1
...
--7. Cuales son los usuarios que tienen entre 10 y 20 armas.
SELECT psn_user 